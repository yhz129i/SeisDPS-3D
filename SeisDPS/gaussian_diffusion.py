...
from SeisDPS.scheduler import get_schedule_jump
from operators import SeisOp
from scipy.linalg import convolution_matrix
import matplotlib.pyplot as plt


def torch_to_np(x_torch): #tp
    return x_torch.squeeze().detach().cpu().numpy()

def soft_thresholding(x, lambd):
    return torch.sign(x) * torch.maximum(torch.abs(x) - lambd, torch.zeros_like(x))

wavelet = np.squeeze(np.load('data/Ricker_Wavelet_30Hz.npy'))
seisop = SeisOp(wavelet, 320).cuda()
H, HT = seisop.Convolve, seisop.Deconvolve
A = lambda x: H(x)
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            ...
            model_mean, model_variance, _ = self.q_posterior_mean_variance(
                x_start=x0_t, x_t=x, t=t
            )


            conf1 = _extract_into_tensor(self.posterior_mean_coef1,
                                     t, x.shape)
            conf2 = _extract_into_tensor(self.posterior_mean_coef2,
                                     t, x.shape)
            sqrt_recip_alphas_cumprod = _extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x.shape) 
            sqrt_recipm1_alphas_cumprod = _extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, t, x.shape)
        ...
        assert (
                model_mean.shape == model_log_variance.shape == x0_t.shape == x.shape 
        )
    ...
    def p_sample(
        ...
        model_mean = out["mean"]
        eps = out["eps"]
        sqrt_recip_alphas_cumprod = out["sqrt_recip_alphas_cumprod"]
        sqrt_recipm1_alphas_cumprod = out["sqrt_recipm1_alphas_cumprod"]
        zeta = 0.5
        xi = 0.005
        lambd = 0.005
        ...
        x0_t = sqrt_recip_alphas_cumprod * x - sqrt_recipm1_alphas_cumprod * eps
        x_t1 = conf1 * x0_t + conf2 * x + nonzero_mask * \
            th.sqrt(th.ones(1,device='cuda')*variance) * noise 
        
        x0_t = soft_thresholding(x0_t, lambd)

        difference = y - A(x0_t)
        norm = torch.norm(difference, p=2) + torch.norm(x0_t, p=1) * xi
        norm_grad = torch.autograd.grad(outputs = norm, inputs=x)[0] 
        with torch.no_grad():
            x_t1_hat = x_t1 - zeta* norm_grad

        result = {"sample": x_t1_hat,
                  "x0_t": x0_t, 'gt': model_kwargs.get('gt'), "norm": norm}
